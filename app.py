...
def get_db_connection():
    try:
        connection = mysql.connector.connect(
            host='localhost',      # Zmień na adres swojej bazy
            user='root',           # Zmień na użytkownika
            password='haslo',      # Zmień na hasło
            database='testdb'      # Zmień na nazwę bazy
        )
...
def home():
    ...
    db_status = "Połączenie z bazą: OK" if conn else "Brak połączenia z bazą!"
    if conn:
        conn.close()
    return f'''
    <!DOCTYPE html>
    ...
    <body>
        <div class="container">
            ...
            <p style="margin-top:20px;font-weight:bold;">{db_status}</p>
